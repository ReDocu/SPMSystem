...
const EMPTY_OPS_TILE = { liveCount: 0, latestDeploy: null, expiringSoon: false };
...
export default async function LauncherPage() {
  ...
  const [inboxCount, resourceCounts, projectCounts, scheduleCounts, opsCounts] = isDbConfigured()
    ? await Promise.all([
        ...
        opsTileCounts(toDateKey(now)).catch((error) => {
          console.error("런처 운영 타일 실패:", error);
          return EMPTY_OPS_TILE;
        }),
      ])
    : [0, { weekCount: 0, ideaCount: 0 }, EMPTY_PROJECT_TILE, EMPTY_SCHEDULE_TILE, EMPTY_OPS_TILE];
  ...
  const TILES = [
    ...
    {
      href: "/ops",
      title: `운영·배포${opsCounts.expiringSoon ? " 🟡" : ""}`,
      main: `운영 중 ${opsCounts.liveCount}`,
      sub: opsCounts.latestDeploy ? `최근 배포 ${opsCounts.latestDeploy}` : "live 전이 시 등장",
    },
    {
      href: "/resources",
      title: "자료수집",
